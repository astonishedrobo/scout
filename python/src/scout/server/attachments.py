    ...
    try:
        ...
        if header:
            ...
            return (
                ...
                f"Use `run_code` with pandas to explore this file."
            )
    except Exception:
        pass
    return (
...
def _handle_excel(p: Path, size_str: str) -> str:
    """XLSX/XLS: count sheets, suggest run_code with openpyxl."""
    try:
        import openpyxl
    ...
    return (
        ...
        f"Path: {p}\n"
        f"Use `run_code` with pandas/openpyxl to explore this file."
    )
        ...
        if isinstance(data, list):
            ...
            if n > 0 and isinstance(data[0], dict):
                ...
                return (
                    ...
                    f"Path: {p}\n"
                    f"Top keys: [{key_str}]\n"
                    f"Use `run_code` with json/pandas to explore, or "
                    f"`search_documents` if it's indexed."
                )
            return (
                f"[Attached: {p.name} (JSON array, {n} items, {size_str})] "
                f"Path: {p}\n"
                f"Use `run_code` to explore this file."
            )
...
def _handle_generic(p: Path, size_str: str) -> str:
    ...
    return (
        ...
        f"Path: {p}\n"
        f"Use `read_file` to peek at content, or `run_code` for binary formats."
    )
