  ...
  async start(): Promise<void> {
    ...
    try {
        ...
        await SandboxManager.initialize(
          {
            filesystem: {
              ...
              denyRead: [`${homedir()}/.ssh`, `${homedir()}/.gnupg`],
              denyWrite: [],
            },
            network: {
              allowedDomains: [
      ...
      if (process.env.SCOUT_DEBUG) {
        process.stderr.write("[sandbox] Sandbox unavailable — proceeding without isolation\n");
      }
    }
    ...
    if (useSandbox) {
      ...
      try {
        ...
        if (process.env.SCOUT_DEBUG) {
          process.stderr.write("[sandbox] wrapWithSandbox failed — spawning without sandbox\n");
        }
